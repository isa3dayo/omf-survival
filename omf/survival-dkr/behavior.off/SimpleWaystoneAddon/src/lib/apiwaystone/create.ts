...
export const apiWaystoneCreate = new class ApiWaystoneCreate {
  createPoint(player: Player, info: WaystoneCreateInfo): string | void {
    ...
    if(!created) return apiWarn.notify(player, "warning.simple_waystone:waystone.failCreateWaystone", {type: "action_bar", sound: "simple_waystone.warn.break"})
    return name
  }
}
