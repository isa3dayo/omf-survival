...
import { world, Player, Block, system, ItemStack } from "@minecraft/server"
...
import { WaystoneInfo } from "../lib/apiwaystone/info"
...
export const colorDimension: { [key: string]: string } = {"overworld": "§2", "nether": "§4", "the_end": "§u"}
const xpSprite = ["", " - \ue701", " - \ue702", " - \ue703", "\ue700"]

export const waystoneUi = new class waystoneUi {
  createPoint(player: Player, block: Block){
    new ModalFormData()
    ...
    .show(player).then(r => {
      ...
      if(typeof name != "string" || typeof access != "boolean") return apiWarn.notify(player, "warning.simple_waystone:waystone.failCreateWaystone", {type: "action_bar", sound: "simple_waystone.warn.break"})

      if(!name) return apiWarn.notify(player, "warning.simple_waystone:waystone.failCreateWaystone", {type: "action_bar", sound: "simple_waystone.warn.break"})
      if(name.replaceAll(/§./g, "").replaceAll("§", "").length < 1) return apiWarn.notify(player, "warning.simple_waystone:waystone.failCreateWaystone", {type: "action_bar", sound: "simple_waystone.warn.break"})
  ...
  waystoneList(player: Player, waystone?: WaystoneInfo, item?: ItemStack){
    ...
    if(!waystones || waystones?.length < 1) return apiWarn.notify(player, "warning.simple_waystone:waystone.failFindWaystones", {type: "action_bar", sound: "simple_waystone.warn.bass"})
    ...
    if(buttons.length < 1) return apiWarn.notify(player, "warning.simple_waystone:waystone.failFindWaystones", {type: "action_bar", sound: "simple_waystone.warn.bass"})
    ...
    form.show(player).then(r => {
      ...
      if(!selected || !button || (selected.name == waystone?.name && apiVec3.compare(selected.pos, waystone.pos))) return apiWarn.notify(player, "warning.simple_waystone:waystone.tpCurrentWaystone", {type: "action_bar"})

      if(player.level < button.cost && player.getGameMode() != "creative") return apiWarn.notify(player, "warning.simple_waystone:waystone.insufficientXp", {sound: "simple_waystone.warn.bass"})

      if(!waystone && item){
        const execute = apiItem[item.typeId]
        if(execute) if(execute(player)) return apiWarn.notify(player, "warning.simple_waystone:waystone.invalidTeleportItem", {type: "action_bar", sound: "simple_waystone.warn.break"})
      }
      if(!item) apiWarn.notify(player, "", {sound: "simple_waystone.block.waystone.teleport", delaySound: 1})
  ...
  settingsMenu(player: Player, waystone: WaystoneInfo){
    const owner = waystone.owner
    if(owner != player.id) return apiWarn.notify(player, "warning.simple_waystone:waystone.notOwner", {sound: "simple_waystone.warn.bass"})
    new ActionFormData()
  ...
  settingsWaystone(player: Player){
    ...
    .submitButton("ui.simple_waystone:waystone.create.button")
    .show(player).then(r => {
      if(r.canceled) return apiWarn.notify(player, "warning.simple_waystone:waystone.cancelSettings", {type: "action_bar", sound: "simple_waystone.warn.break"})
  ...
  removeWaystones(player: Player){
    ...
    if(!allWaystones || allWaystones.length < 1) return

    const myWaystones = allWaystones.filter(way => way.owner == player.id)
    if(myWaystones.length < 1) return apiWarn.notify(player, "warning.simple_waystone:waystone.failFindWaystones", {type: "action_bar", sound: "simple_waystone.warn.bass"})
...
interface ConfigForm extends Array<boolean | number> {
  0: boolean,
  1: number,
  2: number,
  3: number,
