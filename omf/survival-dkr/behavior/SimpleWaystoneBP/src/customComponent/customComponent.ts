...
world.beforeEvents.worldInitialize.subscribe((data) => {
  data.blockComponentRegistry.registerCustomComponent("ws:waystone", {
    ...
    onPlayerInteract: e => {
      ...
      if(perm["ws:waystone_on"] == false) return waystoneUi.createPoint(player, block)

      if(dyes.includes(player.getComponent("equippable")?.getEquipment("Mainhand")?.typeId || "bedrock_awakening:null_item")) return apiWaystoneSpace.paintWaystone(player, block, dyes)
      ...
      if(waystone.owner == player.id && player.getComponent("equippable")?.getEquipment("Mainhand")?.typeId == "minecraft:brush") return apiWaystoneSpace.waystoneToWarpstone(player, block)

      if(apiWaystoneSave.saveClaimWaystone(player, waystone)) return
      if(player.isSneaking) return waystoneUi.settingsMenu(player, waystone)
  ...
  data.itemComponentRegistry.registerCustomComponent("ws:warpstone", {
    onUse: ({source: player, itemStack: item}) => {
      ...
      const cooldownEnd = player.getDynamicProperty("warpstoneCooldown") as number ?? new Date().getTime() / 1000 -1
      if(date < cooldownEnd && item.typeId == "ws:warpstone") return apiWarn.notify(player, {translate: "warning.simple_waystone:warpstone.cooldown", with: [`${cooldownEnd - date}s`]}, {type: "action_bar"})
