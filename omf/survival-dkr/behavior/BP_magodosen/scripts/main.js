...
const FIXED_Y=250, HALF={x:16,y:8,z:16}, STRUCTURE_NAME="magodosen_ship", WORLD_FLAG_KEY="magodosen_ship_placed", MARKER_BLOCK="minecraft:lodestone";
...
function computeCenter(){const s=world.getDefaultSpawnLocation();const y=clamp(FIXED_Y, BUILD_MIN_Y+5, BUILD_MAX_Y-5);return {x:Math.floor(s.x),y,z:Math.floor(s.z)};}
...
world.afterEvents.playerSpawn.subscribe(async ev=>{if(!ev.initialSpawn)return;const p=ev.player,ow=world.getDimension("overworld"),c=computeCenter();
...
if(!(await placedAlready(ow,c))){
  ...
  await ow.runCommandAsync(`fill ${c.x-10} ${c.y+1} ${c.z+10} ${c.y+2} ${c.z+10} oak_fence`);
  await ow.runCommandAsync(`fill ${c.x-10} ${c.y+1} ${c.z-10} ${c.x-10} ${c.y+2} ${c.z+10} oak_fence`);
  await ow.runCommandAsync(`fill ${c.x+10} ${c.y+1} ${c.z-10} ${c.x+10} ${c.y+2} ${c.z+10} oak_fence`);
  await ow.runCommandAsync(`fill ${c.x-1} ${c.y} ${c.z-1} ${c.x+1} ${c.y} ${c.z+1} cobblestone`);
  await ow.runCommandAsync(`execute positioned ${c.x} ${c.y} ${c.z} run function magodosen:lightproof`);
  await ow.runCommandAsync(`execute positioned ${c.x} ${c.y} ${c.z} run function magodosen:place_signs`);
  try{await ow.runCommandAsync(`structure load ${STRUCTURE_NAME} ${c.x} ${c.y} ${c.z}`);}catch{}
  await ow.runCommandAsync(`setworldspawn ${c.x} ${c.y+2} ${c.z}`);
  await markPlaced(ow,c);
}
